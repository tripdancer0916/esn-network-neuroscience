...
class LI_Deep_ESN:
    ...
    def _update(self, state, input_pattern):
        ...
        preactivation = np.dot(self.W[:layer_size, :layer_size], state[:layer_size]) + np.dot(self.W_in[:layer_size],
                                                                                              input_pattern)

        tmp_state[:layer_size] = (1 - self.time_scale[:layer_size]) * state[:layer_size] + self.time_scale[
                                                                                           :layer_size] * np.tanh(
            preactivation)

        for i in range(1, self.num_layer - 1):
            preactivation = np.dot(self.W[layer_size * i:layer_size * (i + 1), layer_size * i:layer_size * (i + 1)],
                                   state[layer_size * i:layer_size * (i + 1)]) \
                            + np.dot(self.W[layer_size * (i - 1):layer_size * i, layer_size * i:layer_size * (i + 1)],
                                     state[layer_size * (i - 1):layer_size * i])

            tmp_state[layer_size * i:layer_size * (i + 1)] = (1 - self.time_scale[
                                                                  layer_size * i:layer_size * (i + 1)]) * state[
                                                                                                          layer_size * i:layer_size * (
                                                                                                                  i + 1)] + self.time_scale[
                                                                                                                            layer_size * (
                                                                                                                                    i - 1):layer_size * i] * np.tanh(
                preactivation)
...
if __name__ == '__main__':
    ...
    mu_list = np.arange(0, 0.04, 0.005)
    mc_mean_list = []
    mc_std_list = []
    for mu in mu_list:
    ...
    if args.layered:
        np.savetxt('./mc-result/result_rsig_{}_averagedegree_{}_ncom_{}_deeplayered.out'.
